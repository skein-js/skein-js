...
| Parity with a dependency                                                                                               | Diff against the **installed** package, not the docs. `sdk-body-parity.test.ts` extracts the keys the SDK's client puts on the wire and requires each to be named in the matching schema — the guard `.passthrough()` otherwise makes impossible.                  |
