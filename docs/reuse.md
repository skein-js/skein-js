...
So Skein is deliberately *thin*. We stand on the OSS runtime, checkpointers, parser,
...
| --- | --- | --- |
| Durable persistence of **protocol resources** (assistants / threads / runs / store rows) | `@skein/storage-postgres` + `SkeinStore` | The OSS server keeps these in memory; production needs Postgres (+ pgvector). Graph *checkpoints* still reuse `PostgresSaver`. |
...
See [code-practices.md](./code-practices.md) for how we keep the code we *do* write small
