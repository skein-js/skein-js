...
| Long-term store interface     | `@langchain/langgraph-checkpoint`            | `BaseStore` in both directions: `SkeinBaseStore` exposes skein's store to graphs as one, and `fromBaseStore` accepts anyone's as skein's store. `InMemoryStore` is the conformance target for that adapter.                                                                                                             |
