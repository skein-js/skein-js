...
// Mirrors packages/server-express/src/express-adapter.test.ts (same conformance oracle).
...
describe("@skein-js/fastify adapter", () => {
  ...
  it("tolerates an empty body sent with content-type application/json (treats it as {})", async () => {
    running = await startEchoServer();
    const { baseUrl } = running;

    // Fastify's default JSON parser 400s on an empty body; the adapter's tolerant parser treats it
    // as `{}` (like express.json()), so a bodyless POST with a JSON content-type still succeeds.
    const res = await fetch(`${baseUrl}/threads`, { method: "POST", headers: jsonHeaders });
    expect(res.status).toBe(200);
    expect(typeof ((await res.json()) as { thread_id: string }).thread_id).toBe("string");
  });
