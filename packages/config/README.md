...
- **`graphs.load(id)`** resolves a `"./path:export"` entry to a runnable graph. It mirrors the LangGraph CLI's `resolveGraph` exactly — splits on the first colon, falls back to the `default` export, compiles an uncompiled `StateGraph`, unwraps the `createAgent` wrapper, and returns a factory export un-invoked so per-run config still applies — so a project moves onto Skein with **no code change**. Loads are cached; a failed load is not memoized, so a transient error can be retried.
...
Skein-specific `skein.json` overrides and driver selection are planned on top.

## Usage

```ts
import { loadConfig } from "@skein/config";
