...
/** Marker every skein-generated asset carries, so we only ever overwrite our own files. */
const GENERATED_MARKER = "# Generated by skein";

/**
 * Whether `contents` is a skein-generated asset. The marker is normally the first line, but the
 * Dockerfile leads with a `# syntax=…` parser directive (which must be line 1), so scan the first
 * few lines rather than only the first.
 */
function isSkeinGenerated(contents: string): boolean {
  return contents.split("\n", 3).some((line) => line.startsWith(GENERATED_MARKER));
}
...
function writeGeneratedFile(file: string, contents: string): "wrote" | "kept-user" {
  ...
  if (existsSync(file) && !isSkeinGenerated(readFileSync(file, "utf8"))) {
