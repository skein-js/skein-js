// A running skein NestJS server backed by a zero-setup echo graph and in-memory drivers, so a test
// is one call away from exercising the real adapter over HTTP. Uses the injected-`deps` path (no
// langgraph.json on disk needed) — the same fixture shape as the Express/Fastify adapters'.
...
import { createNestServer, type SkeinNestServer } from "../create-nest-server.js";
