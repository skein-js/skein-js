// The headline conformance check: drive the running skein-js Fastify server with the real
// `@langchain/langgraph-sdk` client. If the official client's `threads.create` / `runs.wait` /
// `runs.stream` are happy against our Fastify adapter, the wire format is right.
...
describe("fastify-basic over @langchain/langgraph-sdk", () => {
